...
FLAG = 0 # 是否开启此功能
TIMES = 4 # 投放次数
RELEASE_POINTS = [MAX_TIME * random() for i in range(TIMES)] # 投放时间列表
...
def genData(length=70, mutualTest=False):
    ...
    for i in range(length):
        if reset_cnt < 20 and randint(1, 10) < 2:  # 37分出reset
            id = chooseElevator()
            ...
            reset_cnt += 1
            ans.append((time, f"[{time:.1f}]RESET-DCElevator-{id}-{transfer}-{capacity}-{speed}\n"))
