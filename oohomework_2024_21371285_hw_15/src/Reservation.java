...
public class Reservation {
    ...
    private final HashMap<LibraryRequest, Boolean> handledReqs2Recorded = new HashMap<>();
    ...
    public void handleOrder(LibraryReqCmd reqCmd) {
        ...
        if (bookId.isFormal() && facebook.canOrder(bookId, studentId)) {
            facebook.orderNewBook(studentId, bookId);
    ...
    public void handlePick(LibraryReqCmd reqCmd) {
        ...
        if (!bookByUser.containsKey(req.getStudentId())) {
            ...
            // 取书成功记录预约结束
            facebook.getOrderedBook(req.getStudentId(), req.getBookId());
            ...
            while (it.hasNext()) {
                ...
                if (request.getStudentId().equals(req.getStudentId()) &&
                    ...
                    handledReqs2Recorded.remove(request);
                    it.remove();
                    break;
                }
            }
    ...
    public LinkedList<LibraryMoveInfo> returnExpired(LocalDate cutDate) {
        ...
        while (it.hasNext()) {
            ...
            if (date.isBefore(cutDate)) {
                ...
                handledReqs2Recorded.remove(req);
                it.remove();
            } else {
                break;
            }
    ...
    public LinkedList<LibraryMoveInfo> prepareOrdered(LocalDate date) {
        ...
        while (it.hasNext()) {
            ...
            if (bookshelf.hasBook(req.getBookId())) {
                ...
                handledReqs2Recorded.put(req, false);
                it.remove();
    ...
    public void recordExpired(LocalDate date) {
        LocalDate cmpDate = date.minusDays(4);
        for (Pair<LocalDate, LibraryRequest> pair : handledReqs) {
            LocalDate tmpDate = pair.getKey();
            LibraryRequest req = pair.getValue();
            if (tmpDate.isBefore(cmpDate)) {
                if (!handledReqs2Recorded.get(req)) {
                    facebook.minusCreditScore(req.getStudentId(), 3);
                    handledReqs2Recorded.replace(req, true);
                }
            } else {
                break;
            }
