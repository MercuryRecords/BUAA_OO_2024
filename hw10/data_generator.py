...
MAX_M_VALUE = 200
MAX_AGE = 200
instrs = ['ap', 'ar', 'mr', 'qv', 'qci', 'qbs', 'qts', 'at', 'att', 'dft', 'qtvs', 'qtav', 'qba', 'qcs', 'qsp', 'qsp',
          'qsp']
...
def normal_data(instr_num, people_num, tag_num):
    instrlist = []
    instrlist.append(ln_generator(100, True if random() < 0.5 else False))
    instr_num -= 1
    ...
    for i in range(instr_num - people_num - tag_num):
        ...
        if instr == 'ap':
            ...
            person_id = randint(1, people_num)
            instrlist.append(
                triplet_arg_instr(instr, randint(1, people_num), person_id, locate_tag(person_id, people_num, tag_num)))
...
def extend_data(instr_num, people_num, tag_num):
    ...
    for i in range(instr_num - people_num - tag_num):
        ...
        if instr == 'ap':
            ...
            person_id = randint(1, people_num)
            instrlist.append(triplet_arg_instr(instr, randint(1, people_num), person_id,
                                               locate_tag_overlapped(person_id, people_num, tag_num)))
        elif instr == 'qtvs' or instr == 'qtav':
            person_id = randint(1, people_num)
            instrlist.append(twin_arg_instr(instr, person_id, locate_tag_overlapped(person_id, people_num, tag_num)))
        elif instr == 'qba':
...
def all_random(instr_num, people_num, tag_num):
    ...
    for i in range(instr_num):
        ...
        if instr == 'ap':
            ...
            instrlist.append(
                triplet_arg_instr(instr, randint(1, people_num), randint(1, people_num), randint(1, tag_num)))
        elif instr == 'qtvs' or instr == 'qtav':
            instrlist.append(twin_arg_instr(instr, randint(1, people_num), randint(1, tag_num)))
        elif instr == 'qba':
            instrlist.append(sigle_arg_instr(instr, randint(1, people_num)))
...
def fuck_u_data(instr_num=3000, n=100, tag_num=120):
    special_instr = ['mr', 'mr', 'mr', 'mr', 'qbs', 'qts', 'att', 'dft', 'qtvs', 'qtvs', 'qtav', 'qtav', 'qcs', 'qcs',
                     'qsp', 'qsp', 'qsp']
    instrlist = []
    instrlist.append(ln_generator(100, True if random() < 0.5 else False))
    for i in range(1, randint(int(tag_num * 0.7), tag_num)):
    ...
    for i in range(instr_num - 1 - tag_num):
        ...
        if instr == 'ap':
            ...
            instrlist.append(
                triplet_arg_instr(instr, randint(1, n), randint(1, n), randint(-MAX_M_VALUE, MAX_M_VALUE * 0.3)))
        elif instr == 'qv' or instr == 'qci' or instr == 'qsp':
