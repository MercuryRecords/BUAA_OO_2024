...
def run_iteration(iteration):
    ...
    with open(stdout_path, "w") as stdout_file:
        datainput_proc = subprocess.Popen([f"./{FEED_PROGRAM}"],cwd=cache_folder, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    checker_output = subprocess.run([f"./{C_NAME}", stdin_path, stdout_path], capture_output=True, text=True).stdout.strip()
